...
(function () {
    ...
    window.onload = function () {
        try {
            if (window.parent.callback.videoWidgetSelectVideo) {
                $G('videoSelect').style.display = 'inline-block';
                $G('videoSelect').addEventListener('click', function () {
                    window.parent.callback.videoWidgetSelectVideo(function (videos) {
                        $G('videoUrl').value = videos[0];
                        createPreviewVideo(videos[0]);
                    });
                });
            }
        } catch (e) {

...
class MP3File
{
    ...
    public static function parseFrameHeader($fourbytes)
    {
        ...
        static $samples = array(
            1 => array(1 => 384, 2 => 1152, 3 => 1152,),             2 => array(1 => 384, 2 => 1152, 3 => 576,),         );
                $b1 = ord($fourbytes[1]);
        $b2 = ord($fourbytes[2]);
        $b3 = ord($fourbytes[3]);
        ...
        $sample_rate_idx = ($b2 & 0x0c) >> 2;        $sample_rate = isset($sample_rates[$version][$sample_rate_idx]) ? $sample_rates[$version][$sample_rate_idx] : 0;
        ...
        $info = array();
        $info['Version'] = $version;        $info['Layer'] = $layer;
    ...
    private static function framesize($layer, $bitrate, $sample_rate, $padding_bit)
    {
        ...
        else             return intval(((144 * $bitrate * 1000) / $sample_rate) + $padding_bit);
    }
}

    ...
    <div>
        ...
        <div class="tw-bg-white tw-rounded">
            ...
            <div class="tw-px-2" v-if="categories.length>0">
                <i class="iconfont icon-category tw-ml-1"></i>
                <a href="javascript:;" class="tw-text-gray-600" :class="{'ub-text-primary':search.categoryId===0}" @click="search.categoryId=0">
                    全部
                </a>
                <a href="javascript:;" class="tw-mr-1 tw-text-gray-600" :class="{'ub-text-primary':search.categoryId===cat.id}" v-for="(cat,catIndex) in categories" :key="catIndex" @click="search.categoryId=cat.id">
                    {{cat.title}}
                </a>
            </div>
